...
export function isolateComputedFields(subschemaConfig: SubschemaConfig): Array<SubschemaConfig> {
  ...
  if (Object.keys(isolatedSchemaTypes).length) {
    return [
      filterBaseSubschema(
        { ...subschemaConfig, endpoint: subschemaConfig.endpoint, merge: baseSchemaTypes },
        isolatedSchemaTypes
      ),
      filterIsolatedSubschema({ ...subschemaConfig, endpoint: subschemaConfig.endpoint, merge: isolatedSchemaTypes }),
    ];
  }
